...
echo "Wait for installing of Macport completed"
...
port select --set python python39 && port select --set python3 python39

# setup fonts
cd -
mkdir .fonts
curl -L -o Fira_Code_v6.2.zip https://github.com/tonsky/FiraCode/releases/download/6.2/Fira_Code_v6.2.zip
...
# compile neovim
port install ninja cmake autoconf automake gettext xsel
cd $TMPDIR
git clone https://github.com/neovim/neovim.git && cd neovim
make install CMAKE_BUILD_TYPE=Release

# setup fish shell
port install fish && chpass -s /opt/local/bin/fish ${USER} && sudo sh -c 'echo /opt/local/bin/fish >> /etc/shells'
